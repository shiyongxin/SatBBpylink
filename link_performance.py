...
def sp_link_performance():  # this function runs the availability for a single point and shows a complete output
    ...
    print('Link budget at 0.1%:', file=sys.stdout)
    print('', file=sys.stdout)
    print('C/N0: ', satelite.get_c_over_n0(0.1), ' dB')
    print('SNR: ', satelite.get_snr(0.1), ' dB')
    print('', file=sys.stdout)
    print('', file=sys.stdout)

    print('Actual SNR target\'s availability: ', satelite.get_availability(0.5, snr_relaxation), '%',
          file=sys.stdout)
    print('SNR at Above availability:', satelite.get_snr(100-(satelite.get_availability(0.5, snr_relaxation))), 'dB', file=sys.stdout)
    print('', file=sys.stdout)
